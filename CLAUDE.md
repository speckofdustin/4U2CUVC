...
2. **Camera error tint.** When the camera or server can't be reached (`setStatus(false)`), the Camera field (`#camera-field.cam-error`) takes a light red background mixed from Color Flow's `--flow-invalid`. The normal state is the standard field, with no "connected" indicator. A visually hidden `#camera-status` (role=status, referenced by the select's `aria-describedby`) announces "Camera unavailable".
